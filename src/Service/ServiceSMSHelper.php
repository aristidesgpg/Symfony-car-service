...
class ServiceSMSHelper
{
    ...
    public function getThreadsByAdvisor($userId, $searchTerm = null, $isShared = null)
    {
        ...
        if ($isShared) {
            $threadQuery = "SELECT * FROM (SELECT c.id, c.name, c.phone, ss3.date,ss3.message, ss4.unread, ss2.repairOrderID, ss2.repairOrderNumber from (SELECT  * from (SELECT * from repair_order where primary_advisor_id = " . $userId . " group by primary_advisor_id ,primary_customer_id) ss 
